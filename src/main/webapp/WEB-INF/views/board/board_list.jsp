			...
			<table width="80%" border="1" cellspacing="0" align="center">
				...
				<c:forEach var="board" items="<%=list%>">
					<tr onmouseover="this.style.backgroundColor='gray'" onmouseout="this.style.backgroundColor='white'">
						...
						<td align="left">
							...
							<a href="${ path }/BoardContent.board?idx=${board.idx}&cp=${cpage}&ps=${pagesize}">
				...
				<tr>
					...
					-->
						...
						<!--이전 링크 --> 
						<c:if test="${cpage > 1}">
							<a href="${ path }/BoardList.board?cp=${cpage-1}&ps=${pagesize}">이전</a>
						</c:if>
						<!-- page 목록 나열하기 -->
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="${ path }/BoardList.board?cp=${i}&ps=${pagesize}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cpage < pagecount}">
							<a href="${ path }/BoardList.board?cp=${cpage+1}&ps=${pagesize}">다음</a>
						</c:if>
					</td>
				...
				<tr>
					...
					<%
						...
						ThePager pager = new ThePager(totalboardcount,cpage,pagesize,pagersize,requestpath+"/BoardList.board");
					%>
					<%= pager.toString() %>
					</td>
			</table>
